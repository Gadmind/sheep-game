#!/usr/bin/env node
/**
 * 游戏大厅 - Next.js 自定义服务器 + Socket.IO 五子棋联机
 * 使用方法：node server.js 或 npm run dev
 * 默认端口：3000（Next.js 默认）
 */
...
function getRoomListForSocket(io, socketId) {
    const validRooms = Array.from(rooms.entries())
        .filter(([, room]) => {
            if (!isSocketConnected(io, room.black)) return false;
            if (room.white && !isSocketConnected(io, room.white)) return false;
            return true;
        })
        .map(([roomId, room]) => ({
            roomId,
...
function broadcastRoomList(io) {
    io.in(LOBBY).fetchSockets().then(sockets => {
        sockets.forEach(s => s.emit('roomList', getRoomListForSocket(io, s.id)));
    });
}

/** 玩家离开当前所在的游戏房间（排除 excludeRoomId），确保只能在一个房间内 */
function leaveOtherGameRooms(io, socket, excludeRoomId) {
